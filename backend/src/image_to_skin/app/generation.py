...
def generate_skin(prompt, num_inference_steps=40, guidance_scale=9, model_precision_type=None, seed=0) -> Image:
    # Enable GPU acceleration frameworks, if enabled.

    if not model_precision_type:
    ...
    print("Loading HuggingFace model: '{}'.".format(MODEL_NAME))
    if device == "cpu":
    ...
    print("Generating skin with prompt: '{}'.".format(prompt))
    ...
    print("Extracting and scaling Minecraft skin from generated image.")

    return extract_minecraft_skin(generated_image)
